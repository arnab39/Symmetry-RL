...
ModelOptInfo = namedtuple("OptInfo", ["loss",
                                      "gradNorm",
                                      "tdAbsErr",
                                      "modelRLLoss",
                                      "RewardLoss",
...
class GroupCategoricalDQN(CategoricalDQN):
    """Distributional DQN with fixed probability bins for the Q-value of each
    action, a.k.a. categorical."""

    def __init__(self,
                 groupeqv_loss_weight=1.,
                 reward_loss_weight=1.,
                 acteqv_loss_weight=1.,
                 time_offset=0,
                 distributional=1,
                 jumps=0,
                 **kwargs):
        super().__init__(**kwargs)
        self.opt_info_fields = tuple(f for f in ModelOptInfo._fields)
        self.groupeqv_loss_weight = groupeqv_loss_weight
        self.acteqv_loss_weight = acteqv_loss_weight
    ...
    def optimize_agent(self, itr, samples=None, sampler_itr=None):
        ...
        for _ in range(self.updates_per_optimize):
            ...
            loss, td_abs_errors, reward_loss,\
            group_eqv_loss, action_eqv_loss = self.loss(samples_from_replay)

            group_loss = self.acteqv_loss_weight * action_eqv_loss \
                        + self.groupeqv_loss_weight * group_eqv_loss
            total_loss = loss + self.reward_loss_weight * reward_loss
            ...
            opt_info.ModelGroupEqvLoss.append(group_eqv_loss.item())
            opt_info.ModelActEqvLoss.append(action_eqv_loss.item())
            opt_info.TotalGroupLoss.append(group_loss.item())
    ...
    def loss(self, samples):
        ...
        """
        if self.model.noisy:
            self.model.rl_head.reset_noise()
        # start = time.time()
        log_pred_ps, reward_loss, action_eqv_loss, group_eqv_loss\
        ...
        action_eqv_loss = action_eqv_loss * nonterminals

        if self.jumps > 0:
            action_eqv_loss = action_eqv_loss[1:].mean(0)
        else:
            action_eqv_loss = torch.zeros_like(action_eqv_loss)
        action_eqv_loss = action_eqv_loss.cpu()
        reward_loss = reward_loss.cpu()
        group_eqv_loss = group_eqv_loss.cpu()

        if self.prioritized_replay:
            weights = samples.is_weights
            action_eqv_loss = action_eqv_loss * weights
            reward_loss = reward_loss * weights
            group_eqv_loss = group_eqv_loss * weights
