...
from src.utils import save_checkpoint, load_checkpoint, save_latent_animation, save_screen_animation
...
class MinibatchRlEvalWandb(MinibatchRlEval):
    ...
    def __init__(self, final_eval_only=False,
                 ckpt_path = '/home/arnab39/SymmetryRL/checkpoints/test',
                 visualization_path = '/home/arnab39/SymmetryRL/visualization',
                 save_ckpt = True,
                 *args, **kwargs):
        ...
        for itr in range(n_itr):
            ...
            with logger.prefix(f"itr #{itr} "):
                ...
                if (itr + 1) % self.log_interval_itrs == 0:
                    ...
                    if itr == 99999 and self.save_ckpt:
                        try:
                            model = self.agent.model.module
                        except:
                            model = self.agent.model
                        save_checkpoint({
                            'itr': itr,
                            'model_state_dict': model.state_dict()
                            }, self.ckpt_path + f'_{itr}'
                        )
        self.shutdown()
    ...
    def visualize(self):
        ...
        try:
            ...
            model = self.agent.model
        checkpoint = load_checkpoint(self.ckpt_path + f'_99999')
        ...
        for itr in range(n_itr):
            with logger.prefix(f"itr #{itr} "):
                ...
                if samples is not None:
                    samples_to_buffer = self.algo.samples_to_buffer(samples)
                    self.algo.replay_buffer.append_samples(samples_to_buffer)
                    samples = self.algo.replay_buffer.sample_batch(64)
                    if itr == 150:
                        observation = samples.all_observation.to(self.agent.device)
                        #[sequencelength, batchsize, channels, 1, imageshape, imageshape]
                        sample_num = random.randint(0, observation.shape[1]-1)
                        print(observation.shape, observation.type())
                        for channel_num in range(observation.shape[2]):
                            save_screen_animation(
                                observation[:, sample_num, channel_num, :, :, :].detach().cpu().numpy(),
                                self.visualization_path, channel_num + 1
                            )
                        input_obs = observation[:, sample_num].flatten(1, 2).float() / 255.
                        #input_obs = input_obs.float() / 255. if input_obs.dtype == torch.uint8 else input_obs
                        print(input_obs.shape, input_obs.type())
                        latent = model.stem_forward(input_obs)
                        state_rep_group_param = model.group_head(latent)
                        # shape [jumps, num_blocks * group_param_dim]
                        state_rep_group_param = state_rep_group_param.reshape(-1, model.num_blocks, model.group_param_dim)
                        # shape [jumps, num_blocks, group_param_dim]
                        state_rep_group_rep = model.group_parameterization.get_group_rep(state_rep_group_param)
                        # shape [jumps, num_blocks, group_rep_dim, group_rep_dim]
                        print(state_rep_group_rep.shape)
                        for j in range(state_rep_group_rep.shape[1]):
                            save_latent_animation(
                                    state_rep_group_rep[:, j, :, :].detach().cpu(),
                                    j+1, self.visualization_path
                                )
